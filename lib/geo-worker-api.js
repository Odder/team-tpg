...
const GeoWorkerAPI = {
    ...
    /**
     * Build distance field for heatmap rendering (WASM accelerated)
     * @param {Array} midpoints - Array of {lat, lng}
     * @param {number} resolution - Grid resolution in degrees (e.g., 0.2)
     * @param {Object} options - { onProgress: fn }
     * @returns {Promise<{fieldMeta, elapsed}>}
     */
    async buildDistanceField(midpoints, resolution, options = {}) {
        const { onProgress = null } = options;
        ...
        return new Promise((resolve, reject) => {
            ...
            this.worker.postMessage({
                type: 'buildDistanceField',
                id,
                data: { midpoints, resolution }
            });
        });
    },
