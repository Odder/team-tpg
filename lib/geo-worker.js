...
/**
 * Build distance field using WASM
 */
function buildDistanceFieldWASM(midpoints, resolution) {
    // Convert midpoints to flat array
    const flat = new Float64Array(midpoints.length * 2);
    for (let i = 0; i < midpoints.length; i++) {
        flat[i * 2] = midpoints[i].lat;
        flat[i * 2 + 1] = midpoints[i].lng;
    }

    // Get dimensions
    const dims = wasmModule.get_distance_field_dimensions(resolution);
    const latCells = Number(dims[0]);
    const lngCells = Number(dims[1]);

    // Build the field
    const field = wasmModule.build_distance_field(flat, resolution);
...
self.onmessage = async function(e) {
    ...
    if (type === 'buildDistanceField') {
        const { midpoints, resolution } = data;
        ...
        try {
            ...
            if (wasmReady) {
                // Use WASM
                progressCallback(0);
                fieldMeta = buildDistanceFieldWASM(midpoints, resolution);
                progressCallback(100);
            } else {
                // JS fallback not implemented for distance field - too slow
                // Return error asking to reload for WASM
                throw new Error('Distance field requires WASM. Please reload the page.');
            }
